...
def replace_chevrons(text_list):
    if text_list and type(text_list) is list:
        replaced_texts = [
            text.replace("<", "&lt;").replace(">", "&gt;") for text in text_list
        ]
...
STAGES_WITH_ENEMY_REF_TO_IGNORE = {
    "level_rogue1_4-2": ["enemy_1025_reveng_2"],
...
STAGES_WITH_REF_TO_REPLACE = {
    ...
    "level_rogue5_4-1": "/dlc2/level_rogue5_4-1_dlc2",
    "level_rogue5_4-2": "/dlc2/level_rogue5_4-2_dlc2",
    "level_rogue5_4-3": "/dlc2/level_rogue5_4-3_dlc2",
    "level_rogue5_4-4": "/dlc2/level_rogue5_4-4_dlc2",
    "level_rogue5_4-5": "/dlc2/level_rogue5_4-5_dlc2",
    "level_rogue5_4-6": "/dlc2/level_rogue5_4-6_dlc2",
    "level_rogue5_5-1": "/dlc2/level_rogue5_5-1_dlc2",
    "level_rogue5_5-2": "/dlc2/level_rogue5_5-2_dlc2",
    "level_rogue5_5-3": "/dlc2/level_rogue5_5-3_dlc2",
    "level_rogue5_5-4": "/dlc2/level_rogue5_5-4_dlc2",
    "level_rogue5_5-5": "/dlc2/level_rogue5_5-5_dlc2",
    "level_rogue5_5-6": "/dlc2/level_rogue5_5-6_dlc2",
    "level_rogue5_6-1": "/dlc2/level_rogue5_6-1_dlc2",
    "level_rogue5_6-2": "/dlc2/level_rogue5_6-2_dlc2",
    "level_rogue5_7-1": "/dlc2/level_rogue5_7-1_dlc2",
    "level_rogue5_7-2": "/dlc2/level_rogue5_7-2_dlc2",
    "level_rogue5_b-4": "/dlc2/level_rogue5_b-4_dlc2",
    "level_rogue5_b-4-b": "/dlc2/level_rogue5_b-4-b_dlc2",
    "level_rogue5_b-5": "/dlc2/level_rogue5_b-5_dlc2",
    "level_rogue5_b-5-b": "/dlc2/level_rogue5_b-5-b_dlc2",
    "level_rogue5_b-6": "/dlc2/level_rogue5_b-6_dlc2",
    "level_rogue5_b-7": "/dlc2/level_rogue5_b-7_dlc2",
}
...
def get_trimmed_stage_data(stage_data, meta_info, extrainfo, rogue_topic=None):
    ...
    enemy_list, elite_enemy_list = itemgetter("enemy_list", "elite_enemy_list")(
        wave_data
    )
    ...
    trimmed_stage_info = {
        ...
        "addInfo_zh": replace_chevrons(extrainfo[levelId].get("addInfo_zh", None))
        if levelId in extrainfo
        else None,
        "addInfo_ja": replace_chevrons(extrainfo[levelId].get("addInfo_ja", None))
        if levelId in extrainfo
        else None,
        "addInfo_en": replace_chevrons(extrainfo[levelId].get("addInfo_en", None))
        if levelId in extrainfo
        else None,
        "eliteDesc_zh": replace_chevrons(extrainfo[levelId].get("eliteDesc_zh", None))
        if levelId in extrainfo
        else None,
        "eliteDesc_ja": replace_chevrons(extrainfo[levelId].get("eliteDesc_ja", None))
        if levelId in extrainfo
        else None,
        "eliteDesc_en": replace_chevrons(extrainfo[levelId].get("eliteDesc_en", None))
        if levelId in extrainfo
        else None,
        "all_mods": extrainfo[levelId].get("all_mods", None)
        if levelId in extrainfo
        else None,
        "n_mods": extrainfo[levelId].get("normal_mods", None)
        if levelId in extrainfo
        else None,
        "elite_mods": extrainfo[levelId].get("elite_mods", None)
        if levelId in extrainfo
        else None,
        "floors": extrainfo[levelId].get("floors", None)
        if levelId in extrainfo
        else None,
        "sp_terrain": sp_terrain,
        "sp_enemy": extrainfo[levelId].get("sp_enemy", None)
        if levelId in extrainfo
        else None,
        "n_count": extrainfo[levelId].get("enemy_counts", None)
        if levelId in extrainfo
        else None,
        "e_count": extrainfo[levelId].get("elite_enemy_counts", None)
        if levelId in extrainfo
        else None,
        "traps": [],
    }
    ...
    (
        ...
        "forbid_locations",
        "max_cost",
    )(get_runes_data(stage_data["runes"], levelId, stage_data["mapData"]))
    ...
    if stage_data["predefines"]:
                ...
                traps.append(
                    {
                        ...
                        if "overrideSkillBlackboard" in item
                        else None,
                    }
                )
        for item in stage_data["predefines"]["tokenCards"]:
            if item["hidden"]:
                continue
            token_cards.append(
                {"key": item["inst"]["characterKey"], "count": item["initialCnt"]}
            )
        ...
        if enemy_id in my_enemy_db:
            ...
            if enemy["overwrittenData"]:
                ...
                if enemy["overwrittenData"]["lifePointReduce"]["m_defined"]:
                    ...
                    if (
                        levelId == "level_rogue5_t-2"
                        and enemy_id == "enemy_2034_sythef"
                    ):
                        overwrittenData["lifepoint"] = 1
                if (
                    "notCountInTotal" in enemy["overwrittenData"]
                    and enemy["overwrittenData"]["notCountInTotal"]["m_defined"]
                    and enemy["overwrittenData"]["notCountInTotal"]["m_value"]
                ):
                    if my_enemy_db[enemy_id]["notCountInTotal"] is False:
                        ...
                        overwrittenData["talentBlackboard"].append(
                            {
                                "key": "not_count_in_total",
                                "tooltip": {
                                    "en": [
                                        "$【Non-Key Target】$Will not prevent ending of battle even when still on field"
                                    ],
                                    "ja": [
                                        "$【非重要目標】$戦場にいても作戦終了に影響しない"
                                    ],
                                    "zh": [
                                        "$【非首要目标】$该目标在场不会阻止战斗结束"
                                    ],
                ...
                if enemy["overwrittenData"]["rangeRadius"]["m_defined"]:
                    overwrittenData["range"] = enemy["overwrittenData"]["rangeRadius"][
                        "m_value"
                    ]
                if (
                    "levelType" in enemy["overwrittenData"]
                    and enemy["overwrittenData"]["levelType"]["m_defined"]
                ):
                ...
                if (
                    ...
                    enemy["overwrittenData"]["talentBlackboard"]
                    or enemy["overwrittenData"]["skills"]
                ):
                    if (
                        levelId not in talent_overwrite_list
                        and enemy["id"] != "enemy_1106_byokai_b"
                    ):
                        if is_unhandled_alert(levelId, enemy_id, rogue_topic):
                            alerts.append(
                ...
                if enemy["overwrittenData"]["talentBlackboard"]:
                    ...
                    for item in enemy["overwrittenData"]["talentBlackboard"]:
                        if (
                            item["key"].casefold() == "yinyang.dynamic"
                            and item["value"] == 0
                        ):
                            overwrittenData["talentBlackboard"].append(
                            ...
                            and item["key"] == "Combat.enemy_key"
                        ):
                            overwrittenData["talentBlackboard"].append(
                                {"key": "transform", "value": item["valueStr"]}
                            )
                        elif (
                            levelId == "level_rogue4_b-8"
                            and item["key"] == "summon.enemy_key"
                        ):
                            overwrittenData["talentBlackboard"].append(
                                {"key": "transform", "value": item["valueStr"]}
                            )
    ...
    map_waves_data = get_waves_data(alt_data or stage_data, levelId, log=False)
...
def generate_roguelike_stages():
    ...
    for topic_dict in roguelike_topics:
        for stage_key in cn_roguelike_topic_table["details"][topic_dict["topic"]][
            "stages"
        ]:
            to_skip = False
...
def generate_normal_stages(topic):
    ...
    activities_with_story = [
        id
        for id in cn_story_review_table
        if cn_story_review_table[id]["entryType"]
        in ["MAINLINE", "ACTIVITY", "MINI_ACTIVITY"]
    ]
    activities = []
    stages = {}
    ...
    for stageId in cn_stage_table["stages"]:
        ...
        if stage_info["isStoryOnly"] or stage_info["stageType"] in [
            "CLIMB_TOWER",
            "CAMPAIGN",
            "GUIDE",
        ]:
        ...
        if stage_info["stageType"] == "ACTIVITY":
            activity_id = (
                activity_table["zoneToActivity"][activity_id]
                if activity_id in activity_table["zoneToActivity"]
                else None
            )
            if not activity_id:
                activity_id = next(
                    (id for id in activities_with_story if id in stage_info["zoneId"]),
                    stage_info["zoneId"],
                )
            ...
            activities.append(
                {
                    ...
                    "name": {
                        "zh": cn_story_review_table[activity_id]["name"]
                        if activity_id in cn_story_review_table
                        else None,
                        "ja": jp_story_review_table[activity_id]["name"]
                        if activity_id in jp_story_review_table
                        else None,
                        "en": en_story_review_table[activity_id]["name"]
                        if activity_id in en_story_review_table
                        else None,
                    },
                    "zones": [],
                }
        ...
        meta_info = {
            ...
            "name_ja": jp_stage_table["stages"][stageId]["name"]
            if TOPIC_IN_GLOBAL
            else None,
            "name_en": en_stage_table["stages"][stageId]["name"]
            if TOPIC_IN_GLOBAL
            else None,
        ...
        if not zoneId in stages:
            ...
            zoneNameFirst = {
                ...
                "ja": jp_zone_table["zones"][zoneId]["zoneNameFirst"]
                if zone_in_global
                else None,
                "en": en_zone_table["zones"][zoneId]["zoneNameFirst"]
                if zone_in_global
                else None,
            }
            ...
            zoneNameSecond = {
                ...
                "ja": jp_zone_table["zones"][zoneId]["zoneNameSecond"]
                if zone_in_global
                else None,
                "en": en_zone_table["zones"][zoneId]["zoneNameSecond"]
                if zone_in_global
                else None,
            }
            stages[zoneId] = {
        ...
        stages[zoneId]["stages"].append(
            {
                ...
                "name": {
                    "zh": stage_info["name"],
                    "ja": jp_stage_table["stages"][stageId]["name"]
                    if TOPIC_IN_GLOBAL
                    else None,
                    "en": en_stage_table["stages"][stageId]["name"]
                    if TOPIC_IN_GLOBAL
                    else None,
                },
            }
    ...
    for zoneId in stages:
        activity_id = (
            activity_table["zoneToActivity"][zoneId]
            if zoneId in activity_table["zoneToActivity"]
            else None
        )
        if not activity_id:
            activity_id = next(
