...
def update_crisis_runes():
    ...
    for season in cn_crisisv2_table["recalRuneData"]["seasons"]:
        ...
        for stage_id in season_data["stages"]:
            if stage_id not in crisis_runes_table:
...
def get_crisis_stage_info():
    ...
    try:
        for season in cn_crisisv2_table["recalRuneData"]["seasons"]:
            TOPIC_IN_GLOBAL = (
                "recalRuneData" in en_crisisv2_table
                and season in en_crisisv2_table["recalRuneData"]["seasons"]
            )
            season_data = cn_crisisv2_table["recalRuneData"]["seasons"][season]
            for stage_id in season_data["stages"]:
                stage_info = season_data["stages"][stage_id]
                ...
                meta_info = {
                    ...
                    "name_ja": jp_crisisv2_table["recalRuneData"]["seasons"][season][
                        "stages"
                    ][stage_id]["levelName"]
                    if TOPIC_IN_GLOBAL
                    else None,
                    "name_en": en_crisisv2_table["recalRuneData"]["seasons"][season][
                        "stages"
                    ][stage_id]["levelName"]
                    if TOPIC_IN_GLOBAL
                    else None,
                    "description_zh": stage_info["levelDesc"].replace("\\n", "\n")
                    if stage_info["levelDesc"]
                    else None,
                    "description_ja": jp_crisisv2_table["recalRuneData"]["seasons"][
                        season
                    ]["stages"][stage_id]["levelDesc"].replace("\\n", "\n")
                    if TOPIC_IN_GLOBAL and stage_info["description"]
                    else None,
                    "description_en": en_crisisv2_table["recalRuneData"]["seasons"][
                        season
                    ]["stages"][stage_id]["levelDesc"].replace("\\n", "\n")
                    if TOPIC_IN_GLOBAL and stage_info["description"]
                    else None,
                }
                wave_data = get_wave_spawns_data(stage_data, stage_id, log=False)
                (
                    enemy_list,
                    elite_enemy_list,
                    sp_count,
                ...
                for rune_type in stage_info["runes"]:
                    rune_data = stage_info["runes"][rune_type]
                    rune_blackboard = [
                        rune["blackboard"] for rune in rune_data["packedRune"]["runes"]
                    ]
                    flat_board = [
                        item for sublist in rune_blackboard for item in sublist
                    ]
                    runes.append(
                        {
                            ...
                            "description": {
                                "zh": replace_substrings(
                                    rune_data["packedRune"]["description"], flat_board
                                ),
                                "ja": replace_substrings(
                                    jp_crisisv2_table["recalRuneData"]["seasons"][
                                        season
                                    ]["stages"][stage_id]["runes"][rune_type][
                                        "packedRune"
                                    ]["description"],
                                    flat_board,
                                )
                                ...
                                "en": replace_substrings(
                                    en_crisisv2_table["recalRuneData"]["seasons"][
                                        season
                                    ]["stages"][stage_id]["runes"][rune_type][
                                        "packedRune"
                                    ]["description"],
                            ...
                            "mods": crisis_runes_table[stage_id][rune_data["runeId"]][
                                "mods"
                            ],
                        }
                    )
                ...
                trimmed_stage_info["systems"]["crisis"] = {
                    "fixedRuneSeriesName": {
                        ...
                        "ja": jp_crisisv2_table["recalRuneData"]["seasons"][season][
                            "stages"
                        ][stage_id]["fixedRuneSeriesName"]
                        if TOPIC_IN_GLOBAL
                        else None,
                        "en": en_crisisv2_table["recalRuneData"]["seasons"][season][
                            "stages"
                        ][stage_id]["fixedRuneSeriesName"]
                        if TOPIC_IN_GLOBAL
                        else None,
