    ...
    if en_char_table[id]["talents"]:
        ...
        for talent_index, talent in enumerate(en_char_table[id]["talents"]):
            ...
            if not talent_holder["name_en"]:
                ...
                talent_holder["name_ja"] = jp_char_table[id]["talents"][talent_index][
                    "candidates"
                ][max_candidate_index]["name"]
                ...
                talent_holder["name_en"] = en_char_table[id]["talents"][talent_index][
                    "candidates"
                ][max_candidate_index]["name"]
...
for id in cn_patch_table["patchChars"]:
    levels_data = chara_talents[id]
    if id not in en_patch_table["patchChars"]:
        return_dict[id] = levels_data
        continue
    if en_patch_table["patchChars"][id]["talents"]:
        talents = []
        for talent_index, talent in enumerate(
            en_patch_table["patchChars"][id]["talents"]
        ):
            talent_holder = chara_talents[id]["talents"][talent_index]
            if not talent_holder["name_en"]:
                max_candidate_index = len(talent["candidates"]) - 1
                maxed_talent = talent["candidates"][max_candidate_index]
                talent_holder["name_ja"] = jp_patch_table["patchChars"][id]["talents"][
                    talent_index
                ]["candidates"][max_candidate_index]["name"]
                talent_holder["desc_ja"] = replace_substrings(
                    jp_patch_table["patchChars"][id]["talents"][talent_index][
                        "candidates"
                    ][max_candidate_index]["description"],
                    maxed_talent["blackboard"],
                )
                talent_holder["name_en"] = en_patch_table["patchChars"][id]["talents"][
                    talent_index
                ]["candidates"][max_candidate_index]["name"]
                talent_holder["desc_en"] = replace_substrings(
                    en_patch_table["patchChars"][id]["talents"][talent_index][
                        "candidates"
                    ][max_candidate_index]["description"],
                    maxed_talent["blackboard"],
                )
...
for skill_id in chara_skills:
    ...
    if skill_id not in en_skill_table:
    ...
    for i, level in enumerate(levels):
        ...
        if not level["description_ja"]:
            level["description_ja"] = replace_substrings(
                jp_skill_table[skill_id]["levels"][level_index]["description"],
                jp_skill_table[skill_id]["levels"][level_index]["blackboard"],
            )
            level["description_en"] = replace_substrings(
                en_skill_table[skill_id]["levels"][level_index]["description"],
                en_skill_table[skill_id]["levels"][level_index]["blackboard"],
            )
    return_dict[skill_id] = data
...
for equip_id in curr_uniequip:
    ...
    battle_equip = (
        en_battle_equip_table[equip_id] if equip_id in en_battle_equip_table else None
    )
    ...
    if battle_equip and not curr_uniequip[equip_id]["name_en"]:
        ...
        for phase_idx, phase in enumerate(battle_equip["phases"]):
            ...
            for index, part in enumerate(phase["parts"]):
                ...
                if "TRAIT" in part["target"] or part["target"] == "DISPLAY":
                    ...
                    max_candidate_en = (
                        en_battle_equip_table[equip_id]["phases"][phase_idx]["parts"][
                            index
                        ]["overrideTraitDataBundle"]["candidates"][-1]
                        if in_global
                        else None
                    )
                    max_candidate_jp = (
                        jp_battle_equip_table[equip_id]["phases"][phase_idx]["parts"][
                            index
                        ]["overrideTraitDataBundle"]["candidates"][-1]
                        if in_global
                        else None
                    ...
                    concise_parts.append(
                        {
                            ...
                            "addDesc_zh": combat_data["phases"][phase_idx]["parts"][
                                index
                            ]["addDesc_zh"],
                            "addDesc_ja": replace_substrings(
                                max_candidate_jp["additionalDescription"],
                                max_candidate["blackboard"],
                            )
                            if in_global
                            else "",
                            "addDesc_en": replace_substrings(
                                max_candidate_en["additionalDescription"],
                                max_candidate["blackboard"],
                            )
                            if in_global
                            else "",
                            "overrideDesc_zh": combat_data["phases"][phase_idx][
                                "parts"
                            ][index]["overrideDesc_zh"],
                            "overrideDesc_ja": replace_substrings(
                                max_candidate_jp["overrideDescripton"],
                                max_candidate["blackboard"],
                            )
                            if in_global
                            else "",
                            "overrideDesc_en": replace_substrings(
                                max_candidate_en["overrideDescripton"],
                                max_candidate["blackboard"],
                            )
                            if in_global
                            else "",
                ...
                if "TALENT" in part["target"]:
                    ...
                    max_candidate = part["addOrOverrideTalentDataBundle"]["candidates"][
                        -1
                    ]
                    max_candidate_en = (
                        en_battle_equip_table[equip_id]["phases"][phase_idx]["parts"][
                            index
                        ]["addOrOverrideTalentDataBundle"]["candidates"][-1]
                        if in_global
                        else None
                    )
                    max_candidate_jp = (
                    ...
                    concise_parts.append(
                        {
                            ...
                            "upgradeDesc_zh": combat_data["phases"][phase_idx]["parts"][
                                index
                            ]["upgradeDesc_zh"],
                            "upgradeDesc_ja": replace_substrings(
                                max_candidate_jp["upgradeDescription"],
                                max_candidate["blackboard"],
                            )
                            if in_global
                            else "",
                            "upgradeDesc_en": replace_substrings(
                                max_candidate_en["upgradeDescription"],
                                max_candidate["blackboard"],
                            )
                            if in_global
                            else "",
        ...
        combat_data = {"phases": phases, "tags": tags, "blackboard": blackboard}
    ...
    if equip_id in en_uniequip_table["equipDict"]:
        return_dict[equip_id]["name_ja"] = jp_uniequip_table["equipDict"][equip_id][
            "uniEquipName"
        ]
        return_dict[equip_id]["name_en"] = en_uniequip_table["equipDict"][equip_id][
            "uniEquipName"
        ]
