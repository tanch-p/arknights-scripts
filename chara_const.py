...
data = {"chara_handbook": {}, "groups": {}}

for key in cn_team_table:
    data["groups"][key] = {
        "zh": cn_team_table[key]["powerName"],
        "en": en_team_table[key]["powerName"],
        "ja": jp_team_table[key]["powerName"],
    }
for key in cn_handbook_info["handbookDict"]:
    ...
    if "npc" in key:
        continue
    cn_info = cn_handbook_info["handbookDict"][key]["storyTextAudio"][0]["stories"][0][
        "storyText"
    ]
    # if not '种族' in cn_info:
    #     print(key,"no 种族")
    if "种族" not in cn_info:
        race["zh"] = "其他"
        race["en"] = "Others"
        race["ja"] = "その他"
    else:
        ...
        if key in en_handbook_info["handbookDict"]:
            race["en"] = (
                en_handbook_info["handbookDict"][key]["storyTextAudio"][0]["stories"][
                    0
                ]["storyText"]
                .split("[Race]")[1]
                .split("\n")[0]
            ...
            birthplace["en"] = (
                ...
                .split("[Place of Birth]")[1]
                .split("\n")[0]
                .strip()
            )
            race["ja"] = (
                jp_handbook_info["handbookDict"][key]["storyTextAudio"][0]["stories"][
                    0
                ]["storyText"]
                .split("【種族】")[1]
                .split("\n")[0]
