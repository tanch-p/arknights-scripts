...
for folder in folders:
    ...
    for dirpath, dirnames, filenames in walk(path):
        files.extend(
            [f for (dirpath, dirnames, filenames) in os.walk(path) for f in filenames]
        )
        break
    files.sort()
...
for key in token_list:
    ...
    holder["name_ja"] = jp_char_table[key]["name"] if in_global else ""
    holder["name_en"] = en_char_table[key]["name"] if in_global else ""
    holder["desc_zh"] = cn_char_table[key]["description"]
    holder["desc_ja"] = jp_char_table[key]["description"] if in_global else ""
    holder["desc_en"] = en_char_table[key]["description"] if in_global else ""
    holder["stats"] = [
        {
            "hp": level1["maxHp"],
            "atk": level1["atk"],
            "def": level1["def"],
            "res": level1["magicResistance"],
