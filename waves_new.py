...
def is_countable_enemy(key, stage_data):
    ...
    if enemy["overwrittenData"] is not None:
        if (
            enemy["overwrittenData"]["prefabKey"]["m_defined"]
            or enemy["useDb"] is False
        ):
            enemy_id = enemy["overwrittenData"]["prefabKey"]["m_value"]
        not_countable = (
            my_enemy_db[enemy_id]["notCountInTotal"]
        ...
        if key not in my_enemy_db:
            return False
        return not (my_enemy_db[key]["notCountInTotal"])


def analyze_enemy_spawns(
    waves_data,
    levelId,
    bonus_data,
    difficulty,
    ...
    for wave_index, wave in enumerate(waves_data):
        if (
            bonus_data
            and bonus_data["type"] == "wave"
            and wave_index == bonus_data["wave_index"]
        ):
            continue
        for fragment_index, fragment in enumerate(wave["fragments"]):
            if (
                bonus_data
                and bonus_data["type"] == "fragment"
                and wave_index == bonus_data["wave_index"]
                and fragment_index == bonus_data["frag_index"]
            ):
                continue
            ...
            for action in spawn_actions:
                if action["hiddenGroup"] is not None and (
                    action["hiddenGroup"] != group_name or group_name is None
                ):
                    continue
                enemy_key = action["key"]
                group_key = (
                    action["randomSpawnGroupKey"]
                    if "randomSpawnGroupKey" in action
                    else None
                )
                pack_key = (
                    action["randomSpawnGroupPackKey"]
                    if "randomSpawnGroupPackKey" in action
                    else None
                )
                if group_key:
                    continue
    ...
    absolute_bonus_counts = (
        [item for item in bonus_counts if item not in base_counts]
        if bonus_data
        else None
    )
    ...
    filtered_enemy_list = [
        {"key": key, "min_count": counts["min"], "max_count": counts["max"]}
        for key, counts in min_max_counts.items()
        if "trap" not in key
    ]
        ...
        for group_key, group_data in fragment["groups"].items():
            ...
            for option in group_data["options"]:
                ...
                for action in option["actions"]:
                    ...
                    if actionKey not in option_spawns:
            ...
            min_max_list = [
                {
                    ...
                    "min": 0
                    if has_none_option
                    or len(group_spawns[key]) != len(local_group_spawns)
                    else min(group_spawns[key]),
                    "max": max(group_spawns[key]),
                }
                for key in group_spawns
            ]
            for item in min_max_list:
                if item["key"] not in holder:
    ...
    for group_key in packed_groups:
        ...
        for i, grouping in enumerate(packed_groups[group_key]):
            if type(grouping) is list:
                is_pack = True
                packed_groups[group_key][i] = [
                    action
                    for action in grouping
                    if (
                        action["hiddenGroup"] is None
                        or action["hiddenGroup"] in hidden_groups
                    )
                ]
        if not is_pack:
            pass
            packed_groups[group_key] = [
                action
                for action in packed_groups[group_key]
                if (
                    action["hiddenGroup"] is None
                    or action["hiddenGroup"] in hidden_groups
                )
    ...
    for fragment in spawn_data:
        for group_key, group_data in fragment["groups"].items():
            ...
            for option in group_data["options"]:
                ...
                options.append({"count": count, "packKey": option["packKey"]})
...
def pack_has_group_in_fragment(actions, pack_key):
    ...
    for action in actions:
        if action.get("randomSpawnGroupPackKey") == pack_key and action.get(
            "randomSpawnGroupKey"
        ):
            return True
    return False
...
def get_runes_data(runes, levelId, mapData):
            ...
            if rune["difficultyMask"] == "FOUR_STAR":
                if key == "level_hidden_group_enable":
                    ...
                    if levelId in ["level_rogue2_5-2"]:
                        enemies_to_replace[rune["blackboard"][1]["valueStr"]] = rune[
                            "blackboard"
                        ][0]["valueStr"]
                    else:
                        enemies_to_replace[rune["blackboard"][0]["valueStr"]] = rune[
                            "blackboard"
                        ][1]["valueStr"]
                    ...
                    locations = value.replace("(", "").replace(")", "").split("|")
                ...
                rune["difficultyMask"] == "NORMAL"
                and key == "level_hidden_group_enable"
            ):
    ...
    return {
        ...
        "elite_group_name": elite_group_name,
        "enemies_to_replace": enemies_to_replace,
        "predefine_changes": predefine_changes,
...
def group_resolver(actions):
    ...
    for action in actions:
        group = (
            action["randomSpawnGroupKey"] if "randomSpawnGroupKey" in action else None
        )
...
def get_bonus(stage_data):
    ...
    if type == "wave":
        ...
        return {
            "type": type,
            "wave_index": bonus_wave_index,
            "frag_index": bonus_frag_index,
        }


def get_wave_spawns_data(stage_data, levelId, log=False):
    waves_data = itemgetter("waves")(compress_waves(stage_data, levelId))
    normal_group_name, elite_group_name, enemies_to_replace = itemgetter(
        "normal_group_name", "elite_group_name", "enemies_to_replace"
    ...
    for diff_group in holder:
        ...
        list, counts, absolute_bonus_count = itemgetter(
            "enemy_list", "enemy_counts", "absolute_bonus_count"
        )(analysis)
    ...
    for wave_idx, wave in enumerate(waves):
        ...
        for frag_index, fragment in enumerate(wave["fragments"]):
            ...
            if (
                bonus_data
                and bonus_data["type"] == "fragment"
                and wave_idx == bonus_wave_index
                and frag_index == bonus_frag_index
            ):
                continue
            for action in fragment["actions"]:
                group = (
                ...
                if actionType not in ["SPAWN", "EMPTY"]:
                    if not (
                        stage_id == "level_rogue4_4-10"
                        and action["key"] == "trap_760_skztzs#0"
                    ):
                        continue
                if action["hiddenGroup"] and action["hiddenGroup"] not in [group_name]:
                    continue
            ...
            if len(groups) > 0:
                ...
                for group_key in groups:
                    if key not in permutations:
                        permutations[key] = {}
                    permutations[key][group_key] = len(groups[group_key])
...
def permutate(permutations, log=False):
    ...
    for frag_index in permutations:
        ...
        for group in permutations[frag_index]:
            ...
            groups.append(list(range(permutations[frag_index][group])))
        ...
        for index, fragment in enumerate(wave["fragments"]):
            ...
            for action in fragment["actions"]:
                if action["key"] in ALWAYS_KILLED_KEYS:
                    continue
                if action["key"] != "" and "trap" not in action["key"]:
                ...
                if action["count"] > 1:
                    ...
                    for count in range(action["count"]):
                        ...
                        spawns[spawn_time].append(
                            {
                                ...
                                "randomSpawnGroupPackKey": action[
                        ...
                        if action["dontBlockWave"] is False:
                            if spawn_time not in wave_blocking_spawns:
                                wave_blocking_spawns[spawn_time] = []
                            wave_blocking_spawns[spawn_time].append(
                                {
                                    "key": action["key"],
                                }
                            )
            ...
            prev_phase_time = (
                max(list(wave_blocking_spawns.keys()))
                if len(wave_blocking_spawns) > 0
                else 0
            )
...
def get_waves_data(stage_data, levelId, log=False, test=False):
    routes, waves_data, extra_routes, branches, map_data = itemgetter(
        "routes", "waves", "extra_routes", "branches", "map_data"
    )(compress_waves(stage_data, levelId))
    normal_group_name, elite_group_name, enemies_to_replace = itemgetter(
        "normal_group_name", "elite_group_name", "enemies_to_replace"
    )(get_runes_data(stage_data["runes"], levelId, stage_data["mapData"]))
    if levelId == "level_rogue4_b-8":
    ...
    for diff_group in holder:
        group_name = normal_group_name if diff_group == "NORMAL" else elite_group_name
        max_permutations, permutations = itemgetter("max_permutations", "data")(
            get_group_permutations(
                stage_data,
                group_name,
                bonus_data,
                bonus_frag_index,
        ...
        for permutation in permutations:
            return_data[diff_group]["permutations"].append(permutation)
