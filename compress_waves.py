...
stage_data_path = os.path.join(
    BASE_DIR,
    "cn_data/zh_CN/gamedata/levels/obt/roguelike/ro3/level_rogue3_1-3.json",
)
...
def compress_waves(stage_data, stage_id):
    ...
    for tile_index, tile in enumerate(stage_data["mapData"]["tiles"]):
        ...
        tiles.append(
            [tile["tileKey"], height, mask, bb if len(bb) > 0 else None, buildableType]
        )
        ...
        if route["checkpoints"]:
            for checkpoint in route["checkpoints"]:
                if (
                    ...
                    and checkpoint["time"] > 998
                ):
                    checkpoint["time"] = 600
                    if "rogue5" in stage_id and stage_id not in ["level_rogue5_b-5"]:
                        checkpoint["time"] = 45
    ...
    if stage_id == "level_rogue4_b-7":
        extra_routes[1]["startPosition"] = {"row": 5, "col": 20}
            ...
            for frag_index, fragment in enumerate(wave["fragments"]):
                if frag_index == 0:
                    ...
                    for idx, action in enumerate(fragment["actions"]):
                        key = action["key"]
                        if (
                            key == "enemy_10060_cjbfod"
                            and action["randomSpawnGroupKey"] == "t1"
                        ):
                            indexes.append(idx)
                    fragment["actions"] = remove_by_indexes(
                        fragment["actions"], indexes
