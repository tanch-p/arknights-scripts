...
def get_wave_data(stage_data, stage_id, log=False):
                ...
                for pack in fragment_group[group]:
                    if pack == "none":
                        ...
                        for key in pack_enemies:
                            ...
                            if key not in pack_groups:
                                pack_groups[key] = [count]
                            else:
                                if count not in pack_groups[key]:
                        ...
                        if pack != "none":
                            ...
                            for action in fragment_group[group][pack]:
                                ...
                                if key not in pack_enemies:
                                    ...
                                    pack_enemies[key] += count
                            if (
                                pack_enemy_key not in pack_enemies
                                and 0 not in pack_groups[pack_enemy_key]
                            ):
    ...
    def get_bonus_count(all_possible_enemy_count):
        all_bonus_enemy_count = [count + 1 for count in all_possible_enemy_count]
        all_absolute_bonus_enemy_count = list(
            set(all_bonus_enemy_count).difference(set(all_possible_enemy_count))
        )
        all_absolute_bonus_enemy_count.sort()
        log and print(f"absolute bonus count: {all_absolute_bonus_enemy_count}")
                    ...
                    groups[group][pack].append(
                        {
                            ...
                            "packKey": packKey,
                            "isUnharmfulAndAlwaysCountAsKilled": action[
    ...
    def distill_wave_data(stage_data, difficulty=1):
        ...
        for wave_idx, wave in enumerate(waves):
            for frag_index, fragment in enumerate(wave["fragments"]):
                ...
                for action in fragment["actions"]:
                    ...
                    if (
                        difficulty == 2
                        and hidden_group is not None
                        and hidden_group != elite_group_name
                    ):
                        continue
                    ...
                    if hidden_group is not None and hidden_group == elite_group_name:
                        ...
                        if pack_group is None:
                            pack_group = packKey
                        if pack_group not in groups:
                            groups[pack_group] = {}
                        if packKey not in groups[pack_group]:
        ...
        return {
            ...
            "enemy_list": json.dumps(enemy_list),
            ...
            if rune["difficultyMask"] == "FOUR_STAR":
                if rune["key"] == "level_hidden_group_enable":
                    ...
                    enemies_to_replace.append(
                        {
                            rune["blackboard"][0]["valueStr"]: rune["blackboard"][1][
                                "valueStr"
                            ]
                        }
                    )
    ...
    base_enemy_count, enemy_list, groups_list = itemgetter(
        "base_enemy_count", "enemy_list", "groups_list"
    )(data)
    enemy_list = json.loads(enemy_list)
    groups_list = json.loads(groups_list)

    has_bonus_wave = has_bonus_wave(groups_list) if len(groups_list) > 0 else False
    ...
    if stage_id == "level_rogue4_t-2":  # lazy hack
        base_enemy_count -= 2
    all_possible_enemy_count = get_all_possible_enemy_counts(base_enemy_count, sums)
    ...
    if elite_group_name is not None or len(enemies_to_replace) > 0:
        ...
        all_elite_group_enemy_counts = get_all_group_enemy_counts(elite_groups_list)
        log and pp.pprint(all_elite_group_enemy_counts)
        for group in all_elite_group_enemy_counts:
            for key in group:
                if key == "":
                    continue
                if key not in elite_enemy_list:
        ...
        all_count_permutations = get_all_count_permutations(elite_groups_list)
        sums = all_sums(all_count_permutations)
        ...
        if has_bonus_wave:
            absolute_elite_sp_counts = get_bonus_count(all_possible_elite_enemy_count)

    enemy_list = [
        {
            "key": key,
            "min_count": enemy_list[key]["min_count"],
            "max_count": enemy_list[key]["max_count"],
        }
    ...
    if elite_enemy_list is not None:
        elite_enemy_list = [
            {
                "key": key,
                "min_count": elite_enemy_list[key]["min_count"],
                "max_count": elite_enemy_list[key]["max_count"],
            }
            for key in elite_enemy_list
            if "trap" not in key
        ]
    ...
    return {
        ...
        "sp_count": absolute_sp_counts,
        "elite_sp_count": absolute_elite_sp_counts,
    }
